...
export default {
   ...
   render(h) {
      return h(this.tag, {
         class: [
            this.activeLocap ? this.classActive : [],
            this.animating ? this.classAnimating : []
         ],
         style: this.disabledAuto ? {} : this.useTransform ? { transform: this.transform, transition: this.transition } : { top: this.top, left: this.left, transition: this.transition }
      }, this.$slots.default)
   }
},
install(Vue) {
   Vue.component("vue-swipe-show", this.VueSwipeShow)
}
}
